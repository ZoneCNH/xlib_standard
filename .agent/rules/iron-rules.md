...
| 9 | Traceability 断链 | RULE-TRACE-* / RULE-CORE-004 | ✅ `goalcli traceability-check` / `make traceability-check` |
| 10 | Release 不完整 | RULE-RELEASE-* / RULE-REL-ARTIFACT-* | ✅ `goalcli release-evidence-check` / `release-final-check` |

## P0 Gate 覆盖状态

> 截至当前规则快照，P0 规则均已 active；后续若 registry 重新生成，仍以 `make rules-verify` 和 `make rules-consistency-check` 的实时输出为准。

- **Traceability Gate**：RULE-CORE-004, RULE-TRACE-001, RULE-TRACE-002, RULE-TRACE-ALG-001, RULE-TRACE-ALG-002 由 `goalcli traceability-check` / `make traceability-check` 执行，按 `.agent/traceability/traceability-matrix.md` 校验 Goal → Req → AC → Task → Issue → Commit → PR → Evidence → Release 链路完整性；断链返回退出码 9。
