...
- 提供 Harness Gate 脚本、生成脚本、CI 工作流、contracts、examples、Evidence、release 和复盘模板。

## 验收标准

- `GOWORK=off go test ./...` 和 `GOWORK=off go test -race ./...` 通过。
- `GOWORK=off make release-check` 通过，并以 `CHECK_STATUS=passed` 生成 `release/manifest/latest.json`。
