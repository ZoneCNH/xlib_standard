...
- `tools`：Go、`golangci-lint` 和按 `XLIB_ENABLE_VULNCHECK` 启用状态记录的 `govulncheck` 版本或可用状态。
...
CI、Release Check 和 Security workflow 仅在 `XLIB_ENABLE_VULNCHECK=1` 时安装 `govulncheck`，安装时必须使用固定版本。当前基线是 `golang.org/x/vuln/cmd/govulncheck@v1.3.0`；升级时应同步更新 workflow、发布文档和验证记录。
