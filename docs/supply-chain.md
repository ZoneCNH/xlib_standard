...
CI、Release Check 和 Security workflow 默认不安装或访问 `govulncheck`；只有设置 `XLIB_ENABLE_VULNCHECK=1` 时才安装，并必须使用固定版本。当前基线是 `golang.org/x/vuln/cmd/govulncheck@v1.2.0`；升级时应同步更新 workflow、发布文档和验证记录。
