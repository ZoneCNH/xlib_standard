...
2. **`.agent/rules/registry.yaml`** — 全部 417 条规则的机器化索引，每条带 `id / level / title / source_section / enforced_by / exit_code / status`。
...
- 244 条 indexed-only 规则等待机器化，每条都是一个潜在的 Goal 候选。

## Rejected

- 拒绝直接删除 `.worktree/goal-patch.md` —— 它是设计推导的真实历史，删除会失去可审计性。
- 拒绝把 417 条规则全部拆成单独 `.md` 文件 —— 会触发它自己定义的 RULE-DOC-001（文档膨胀）。
...
- 验证: `python3 -c "import yaml; d=yaml.safe_load(open('.agent/rules/registry.yaml')); assert d['total_rules']==417 and d['p0_count']==119"`
