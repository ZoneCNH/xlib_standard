...
- CI、Release Check 和 Security workflow 仅在 `XLIB_ENABLE_VULNCHECK=1` 时安装 `govulncheck`，且必须使用固定版本；当前基线是 `golang.org/x/vuln/cmd/govulncheck@v1.3.0`。
- 本地缺少 `golangci-lint` 时 `make lint` 必须失败；启用 `XLIB_ENABLE_VULNCHECK=1` 后缺少 `govulncheck` 时 `make security` 必须失败，不得把当前模式下的必需 gate 记录为跳过。
