...
- Release-readiness summary produced for xlibgate adjudication at `.agent/evidence/l2/release-readiness.json` when a downstream release claim is being evaluated.

## Evidence quality

Evidence should name the manifest version, adapter module, selected packs, command or workflow that produced the report, timestamp, and pass/fail status. A missing file or empty placeholder is not passing evidence. Standards templates may include `.gitkeep` files to establish directories, but downstream release claims must include real reports. Do not commit fabricated active `.agent/evidence/l2/release-readiness.json` evidence to xlib-standard or to a template just to satisfy a gate; temporary verification fixtures must be schema-valid, provider-neutral, and cleaned up before commit.
